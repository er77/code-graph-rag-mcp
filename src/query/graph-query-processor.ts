...
export class GraphQueryProcessor {
  ...
  async executeQuery<T = unknown>(query: GraphQuery): Promise<QueryResult<T>> {
    ...
    switch (query.operation) {
      ...
      case 'getRelatedEntities':
        ...
        break;
      case 'findPath':
        result = await this.findShortestPath(
          query.params.fromId as string,
          query.params.toId as string
        ) as T;
        break;
      case 'getSubgraph':
        result = await this.extractSubgraph(
          query.params.rootId as string,
          query.params.depth as number
        ) as T;
        break;
  ...
  private async getEntityById(id: string): Promise<Entity | null> {
    ...
    try {
      const stmt = this.getPreparedStatement(conn, 'getEntity', `
  ...
  private async getRelatedEntitiesRecursive(
    entityId: string,
    depth: number
  ): Promise<Entity[]> {
      ...
      while (queue.length > 0) {
        ...
        if (current.id === toId) {
          ...
          const nodes = await Promise.all(
            current.path.map(id => this.getEntityById(id))
          );
  ...
  private async detectCycleFromEntity(
    startId: string,
      ...
      for (const rel of relationships) {
        if (rel.fromId === id) {
          if (pathSet.has(rel.toId)) {
            ...
            const cycleNodes = await Promise.all(
              path.slice(cycleStart).map(nodeId => this.getEntityById(nodeId))
            );
  ...
  private async analyzeImpact(entityId: string): Promise<ImpactAnalysis> {
    ...
    const directImpacts = await Promise.all(
      Array.from(directImpactIds).map(id => this.getEntityById(id))
    );
  ...
  private async calculateRippleEffect(changes: Change[]): Promise<RippleEffect> {
    const affectedEntities = new Map<string, {
      entity: Entity;
      impactLevel: number;
      reason: string;
    }>();
    ...
    for (const change of changes) {
      ...
      for (const entity of impact.directImpacts) {
        ...
        const impactLevel = change.type === 'deleted' ? 3 : 
                           change.type === 'modified' ? 2 : 1;
